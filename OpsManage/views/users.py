...
def user(request,uid):    
    if request.method == "GET":
        ...
        userPermList = [ u.get('id') for u in list(user.user_permissions.values())]
        for ds in permList:
            if ds.id in userPermList:ds.status = 1
            else:ds.status = 0        
        #获取用户组列表
        groupList = Group.objects.all() 
        userGroupList = [ g.get('id') for g in list(user.groups.values())]
        for gs in groupList:
        ...
        try:
            ...
            if request.POST.getlist('perms[]') is None:
                ...
                userPermList = []
                for perm in list(user.user_permissions.values()):
            ...
            if request.POST.getlist('groups[]') is None:
                ...
                userGroupList = []
                for group in list(user.groups.values()):
                ...
                print('addGroupList:', addGroupList)
                delGroupList = list(set(userGroupList).difference(set(groupList)))
                print('delGroupList:', delGroupList)
                #添加新增的用户组
    ...
    if request.method == "GET":
        try:
            ...
            return render_to_response('users/group_info.html',{"user":request.user,
                                      ...
                                      context_instance=RequestContext(request)) 
        permList = Permission.objects.filter(codename__startswith="can_") 
        groupPerm = [ p.get('id') for p in list(group.permissions.values())]
        try:
            for ds in permList:
        ...
        try:
            ...
            if request.POST.getlist('perms[]') is None:group.permissions.clear()
                ...
                userPermList = []
                for perm in list(group.permissions.values()):
                    userPermList.append(perm.get('id'))
                permList = [ int(i) for i in request.POST.getlist('perms[]')]
                addPermList = list(set(permList).difference(set(userPermList)))
                delPermList = list(set(userPermList).difference(set(permList)))
